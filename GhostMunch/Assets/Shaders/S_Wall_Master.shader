...
Shader "Shader Forge/S_Wall_Master" {
    Properties {
        ...
        _FadeDistance ("FadeDistance", Float ) = 26
...
////// Emissive:
                ...
                float node_1750 = length((_WorldSpaceCameraPos-i.posWorld.rgb));
                fixed4 finalRGBA = fixed4(finalColor,saturate((pow(node_1750,_FadePower)/_FadeDistance)));
...
////// Lighting:
                ...
                float node_1750 = length((_WorldSpaceCameraPos-i.posWorld.rgb));
                fixed4 finalRGBA = fixed4(finalColor * saturate((pow(node_1750,_FadePower)/_FadeDistance)),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG

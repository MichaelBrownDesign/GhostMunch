...
////// Emissive:
                ...
                float3 node_2999 = (_Player1Pos.rgb-i.posWorld.rgb);
                float3 node_791 = (_Player2Pos.rgb-i.posWorld.rgb);
                float3 node_3827 = (_Player3Pos.rgb-i.posWorld.rgb);
                float3 node_7278 = (_Player4Pos.rgb-i.posWorld.rgb);
                float3 node_865 = float3(1,0,1);
                fixed4 finalRGBA = fixed4(finalColor,saturate((pow(length(((((Function_node_9186( node_2999 , node_791 , node_3827 , node_7278 )+i.posWorld.rgb)*node_865)-(i.posWorld.rgb*node_865))*_FadeAxisBias.rgb)),_FadeFallOff)/_FadeLevel)));
...
////// Lighting:
                ...
                float3 node_7278 = (_Player4Pos.rgb-i.posWorld.rgb);
                float3 node_865 = float3(1,0,1);
                fixed4 finalRGBA = fixed4(finalColor * saturate((pow(length(((((Function_node_9186( node_2999 , node_791 , node_3827 , node_7278 )+i.posWorld.rgb)*node_865)-(i.posWorld.rgb*node_865))*_FadeAxisBias.rgb)),_FadeFallOff)/_FadeLevel)),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
